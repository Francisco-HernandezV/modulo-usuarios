...
export default function AdminCatalogos() {
  ...
  const handleAgregar = async (payload) => {
    try {
      ...
      // Tallas puede devolver duplicadas
      if (tabActiva === "tallas") {
        const { duplicadas = [] } = res.data;
        if (duplicadas.length > 0) {
          alert(`Tallas guardadas. Las siguientes ya existían y se omitieron: ${duplicadas.join(", ")}`);
        }
      }
      ...
      alert(error.response?.data?.message || "Error al agregar. Puede que ya exista.");
    }
  };
  ...
  const handleEliminarConfirmado = async () => {
    ...
    try {
      ...
      alert(error.response?.data?.message || "Error al eliminar. Es posible que esté en uso.");
      setConfirmDel(null);
    }
  };

  const handleEditar = (item) => {
    alert(`La edición para "${item.nombre || item.valor}" estará disponible pronto.`);
  };

  // ── Render de filas según tab ──────────────────────────────────────────
  const renderFilas = () => {
    if (datos.length === 0) {
    ...
    // Tallas: filas agrupadas por tipo
    if (tabActiva === "tallas") {
  ...
  // ── Columnas de encabezado según tab ──────────────────────────────────
