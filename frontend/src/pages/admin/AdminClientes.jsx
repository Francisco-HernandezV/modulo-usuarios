...
export default function AdminClientes() {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      setClientes([]);
      setAlert({ type: "error", msg: "Error al cargar el directorio de clientes." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const validate = () => {
    ...
    if (form.telefono && !/^\d{10}$/.test(form.telefono.replaceAll(" ", "")))
    ...
    <AdminLayout pageTitle="Gestión de Clientes" breadcrumb="Clientes">
            ...
            <tbody>
              {clientes.map(c => (
                <tr key={c.id}>
                  ...
                  <td>{c.rfc ? <span className="adm-badge adm-badge-blue">{c.rfc}</span> : "—"}</td>
                  <td style={{ color: "#9ca3af", fontSize: 12 }}>{fmtFecha(c.creado_en)}</td>
                  <td>
                    <div style={{ display: "flex", gap: "8px" }}>
                      <button className="adm-btn adm-btn-ghost adm-btn-sm" onClick={() => openEdit(c)} title="Editar" type="button"><IconEdit /></button>
                      <button className="adm-btn adm-btn-danger adm-btn-sm" onClick={() => setConfirmDel(c.id)} title="Eliminar" type="button"><IconTrash /></button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="adm-modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="adm-modal-body" style={{ maxHeight: "70vh", overflowY: "auto" }}>
              ...
              <div className="adm-form-row">
                <div className="adm-form-group">
                  <label htmlFor="cli_tel">Teléfono</label>
                  <input id="cli_tel" className="adm-input" maxLength={10} value={form.telefono} onChange={e => handleChange("telefono", e.target.value)} />
                </div>
                <div className="adm-form-group">
                  <label htmlFor="cli_email">Correo</label>
      ...
      {confirmDel && (
        <div 
          ...
          role="button"
          tabIndex={0}
          onKeyDown={(e) => handleKeyPress(e, () => setConfirmDel(null))}
          aria-label="Cerrar confirmación"
        >
          <div className="adm-modal" style={{ maxWidth: 380 }} onClick={e => e.stopPropagation()}>
            <div className="adm-modal-header">
              <h3 className="adm-modal-title">Eliminar cliente</h3>
              <button className="adm-modal-close" onClick={() => setConfirmDel(null)} type="button" aria-label="Cerrar"><IconX /></button>
            </div>
            <div className="adm-modal-body">
              <p style={{ fontSize: "13px", color: "#9ca3af" }}>¿Estás seguro de eliminar a este cliente?</p>
            </div>
            <div className="adm-modal-footer">
              <button className="adm-btn adm-btn-ghost" onClick={() => setConfirmDel(null)} type="button">Cancelar</button>
              <button className="adm-btn adm-btn-danger" onClick={() => handleEliminar(confirmDel)} type="button">Eliminar</button>
            </div>
          </div>
        </div>
      )}
